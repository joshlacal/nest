...
#[derive(Clone)]
pub struct JacquardResolver<H = reqwest::Client> {
    http: H,
    ...
    H: HttpClient + Clone + Send + Sync,
{
    ...
    pub fn new_dns(http: H, opts: ResolverOptions) -> Self {
        Self {
            http,
            opts,
    ...
    async fn get_json_bytes(&self, url: Url) -> resolver::Result<(Bytes, StatusCode)> {
        let request = http::Request::builder()
            .uri(url.as_str())
            .body(Vec::new())
            .map_err(|error| IdentityError::transport(url.to_smolstr(), error))?;
        let resp = self
            .http
            .send_http(request)
            .await
        ...
        Ok((Bytes::from(resp.into_body()), status))
    }
    ...
    async fn get_text(&self, url: Url) -> resolver::Result<String> {
        ...
        let request = http::Request::builder()
            .uri(url.as_str())
            .body(Vec::new())
            .map_err(|error| IdentityError::transport(u.clone(), error))?;
        let resp = self
            .http
            .send_http(request)
            .await
        ...
        if resp.status() == StatusCode::OK {
            String::from_utf8(resp.into_body()).map_err(|error| IdentityError::transport(u, error))
        } else {
            Err(IdentityError::http_status(resp.status()))
        }
    }
...
impl<H> IdentityResolver for JacquardResolver<H>
where
    H: HttpClient + Clone + Send + Sync,
{
...
impl<H> HttpClient for JacquardResolver<H>
where
    H: HttpClient + Clone + Send + Sync,
{
...
#[cfg(feature = "streaming")]
impl<H> jacquard_common::http_client::HttpClientExt for JacquardResolver<H>
where
    H: jacquard_common::http_client::HttpClientExt + Clone + Send + Sync,
{
