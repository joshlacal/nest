...
fn test_e2e_scenario_rejects_non_loopback_http() {
    let output = Command::new(env!("CARGO_BIN_EXE_e2e_external_scenario"))
        ...
        .env("CIRCLE_APPVIEW_SERVICE_DID", "did:web:circles.catbird.blue#atproto_circles")
    ...
    assert_eq!(output.status.code(), Some(2), "Must exit with code 2 for invalid non-loopback HTTP config");
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("non-loopback"), "Stderr must explain non-loopback HTTP restriction: {stderr}");
}
