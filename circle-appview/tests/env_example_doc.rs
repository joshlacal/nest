...
fn match_dockerignore_pattern(pattern: &str, path: &str) -> bool {
    ...
    fn seg_match(pat_segs: &[&str], path_segs: &[&str], pat_idx: usize, target_idx: usize) -> bool {
        ...
        if pat_idx < pat_segs.len() && target_idx < path_segs.len() {
            if match_glob_segment(pat_segs[pat_idx], path_segs[target_idx]) {
...
fn test_dockerignore_rules_exclude_sensitive_files() {
    ...
    let dot_ignore = std::fs::read_to_string(&dot_ignore_path)
        .expect(".dockerignore must exist at crate root");
...
fn test_dockerignore_matcher_descendant_negation_and_glob_semantics() {
    ...
    assert!(evaluate_dockerignore(dir_rules, "artifacts/sub/nested/file.txt"));
    assert!(evaluate_dockerignore(dir_rules, "target/debug/circle_appview"));
    assert!(evaluate_dockerignore(dir_rules, "target/release/deps/lib.rlib"));
    ...
    assert!(evaluate_dockerignore(recursive_rules, "nest/circle-appview/config/local.json"));
    assert!(evaluate_dockerignore(recursive_rules, "a/b/c/config/local.yaml"));
    assert!(!evaluate_dockerignore(recursive_rules, "nest/circle-appview/config/remote.json"));
...
**/*.pem
"#;
    assert!(!evaluate_dockerignore(seq_rules, "catbird-atproto/src/lib.rs"));
    assert!(!evaluate_dockerignore(seq_rules, "nest/circle-appview/Cargo.toml"));
    assert!(!evaluate_dockerignore(seq_rules, "nest/circle-appview/.env.example"));
    assert!(evaluate_dockerignore(seq_rules, "nest/circle-appview/.env"));
    assert!(evaluate_dockerignore(seq_rules, "nest/circle-appview/.env.local"));
    assert!(evaluate_dockerignore(seq_rules, "nest/circle-appview/server.pem"));
    assert!(evaluate_dockerignore(seq_rules, "other-service/src/main.rs"));
}
