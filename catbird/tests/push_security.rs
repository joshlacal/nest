...
async fn test_migration_backfill_duplicate_and_quota_tokens() {
    ...
    let active_quota_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM user_devices WHERE did = $1 AND is_active = TRUE"
    )
    .bind(quota_did)
    .fetch_one(pool)
    .await
    .unwrap();
    assert_eq!(active_quota_count, 10, "Quota excess backfill must leave exactly 10 active devices");
...
async fn test_true_concurrent_token_claims_race() {
    ...
    for i in 0..concurrency {
        ...
        handles.push(tokio::spawn(async move {
            ...
            barrier.wait().await;
            services.registry.upsert_registration(&session, &input).await
        }));
    }
    ...
    .await
    .unwrap();
    assert_eq!(active_count, 1, "Exactly one active registration must exist for token");
...
async fn test_long_send_two_claimers_and_heartbeat_cas() {
    ...
    for token in &[token1, token2] {
        ...
        services.registry.upsert_registration(&session, &reg_input).await.unwrap();
    }
    ...
    assert_eq!(sends.len(), 2, "Both devices must be delivered even when heartbeat renewal fires mid-batch");
    drop(sends);
    ...
    let remaining_rows: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM push_event_queue")
        ...
        .await
        .unwrap();
    assert_eq!(remaining_rows, 0, "Delivered event must be deleted with final heartbeat version");
    ...
    for token in &[token1, token2] {
        ...
        services.registry.upsert_registration(&session_a2, &reg_input).await.unwrap();
    }
    sqlx::query(
        r#"
        INSERT INTO actor_moderation_verdict (recipient_did, actor_did, verdict, display_label, fetched_at)
    ...
    let renewed_v2 = services.queue.extend_lease(row_race.id, token_race, snapshotted_v1, 30).await.unwrap().unwrap();
    assert_eq!(renewed_v2, snapshotted_v1 + 1);

    let fence_res = services.verify_pre_send_fence(
        recipient_did_a2,
        "did:plc:actor",
        token1,
        "like",
    ...
    assert_eq!(fence_res, PreSendFenceOutcome::Authorized, "In-flight fence with stable lease token must succeed across heartbeat renewals");
    ...
    for token in &[token1_b, token2_b] {
        ...
        services.registry.upsert_registration(&session_b, &reg_input).await.unwrap();
    }
    sqlx::query(
        r#"
        INSERT INTO actor_moderation_verdict (recipient_did, actor_did, verdict, display_label, fetched_at)
    ...
    let fake_apns_w1 = Arc::new(ObservingFakeApns::with_barriers(enter_b.clone(), exit_b.clone()));
    ...
    let worker1_handle = tokio::spawn(async move {
        s1_clone.process_queue_batch(&st1_clone).await
    });
    ...
    assert_eq!(w1_sends.len(), 1, "Worker 1 must have aborted delivery after device 1 when lease was lost");
    drop(w1_sends);

    // Worker 2's claimed row must still exist in push_event_queue (Worker 1 must not have deleted it)
    let w2_row_exists: Option<uuid::Uuid> = sqlx::query_scalar("SELECT lease_token FROM push_event_queue WHERE id = $1")
        .bind(row_w2.id)
        .fetch_optional(&pool)
        .await
        .unwrap();
    assert_eq!(w2_row_exists, row_w2.lease_token, "Worker 2's row must remain intact in push_event_queue");
}
...
async fn test_mid_send_auth_revocation_serialization_barrier() {
    ...
    let mut apns_barrier = ObservingFakeApns::with_barriers(enter_barrier.clone(), exit_barrier.clone());
    apns_barrier.sends = fake_apns.sends.clone();
    let apns_barrier = Arc::new(apns_barrier);

    let services_barrier = Arc::new(make_services(pool.clone()).with_apns_sender(apns_barrier.clone()));
    ...
    assert_eq!(processed_revoked, 0, "claim_ready must skip rows for revoked accounts");
    ...
    services.registry.activate_account_session(recipient_did, &new_session.id.to_string(), &new_session.pds_url).await.unwrap();
    ...
    assert_eq!(processed_reauth, 1, "Worker must claim and process the queued row after reauth");
...
async fn test_mid_send_activity_unsubscribe_serialization_barrier() {
    ...
    services.subscriptions.put(subscriber_did, subject_did, &sub).await.unwrap();
    ...
    let mutator_handle = tokio::spawn({
        ...
        async move {
            let unsub = ActivitySubscriptionPreference { post: false, reply: false };
            s.subscriptions.put(&sub_did, &subj_did, &unsub).await.unwrap();
            done.store(true, Ordering::SeqCst);
        }
    });

    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    assert!(!mutator_done.load(Ordering::SeqCst), "Unsubscribe must be blocked by advisory lock during APNs send");
...
async fn test_mid_send_thread_mute_serialization_barrier() {
    ...
    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    assert!(!mutator_done.load(Ordering::SeqCst), "mute_thread must be blocked by advisory lock during APNs send");
...
async fn test_enqueue_before_logout_claim_after_reauth() {
    ...
    services.registry.mark_auth_revoked(recipient_did).await.unwrap();
    ...
    let remaining_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM push_event_queue")
        ...
        .await
        .unwrap();
    assert_eq!(remaining_count, 0, "Rejected stale event must be deleted from queue");
}
...
async fn test_concurrent_quota_final_slot_race_and_fanout() {
    ...
    for h in handles {
        let res = h.await.unwrap();
        assert!(res.is_ok(), "Upsert registration in quota race must succeed: {:?}", res);
    }

    let active_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM user_devices WHERE did = $1 AND is_active = TRUE")
        .bind(did)
        .fetch_one(&pool)
        .await
        .unwrap();
    ...
    let active_registrations = services.registry.list_active_registrations(did).await.unwrap();
    assert!(
...
async fn test_strict_moderation_validation_and_cache_preservation() {
    ...
    assert!(ModerationVerdict::try_from_viewer_json(Some(&json!({"mutedOnlyReposts": null}))).is_err());
    assert!(ModerationVerdict::try_from_viewer_json(Some(&json!({"mutedOnlyQuoteposts": null}))).is_err());
    ...
    services.registry.activate_account_session(recipient_did, &session_live.id.to_string(), &session_live.pds_url).await.unwrap();
    ...
    let fetch_handle = tokio::spawn(async move {
        mod_resolver.resolve(&st_clone, &r_did, &a_did).await
    });

    // Wait 100ms for HTTP fetch to be in flight, then simulate user muting/blocking (invalidation)
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    services.moderation.invalidate_recipient(recipient_did).await.unwrap();
...
async fn test_fenced_lease_expiry_and_false_cas() {
    ...
    // Extend lease
    let v2 = queue.extend_lease(row.id, token, version, 30).await.unwrap().unwrap();
    assert_eq!(v2, version + 1);

    // Attempting CAS with old version must return Ok(false) / None
    let stale_extend = queue.extend_lease(row.id, token, version, 30).await.unwrap();
    assert_eq!(stale_extend, None);

    let stale_retry = queue.retry_later_fenced(row.id, token, version, 1, "test_err").await.unwrap();
    assert!(!stale_retry);
    ...
    // Expire lease
    sqlx::query("UPDATE push_event_queue SET leased_until = NOW() - INTERVAL '1 second' WHERE id = $1")
        .bind(row.id)
        .execute(&pool)
        .await
        .unwrap();
    ...
    assert!(!expired_delete, "delete_fenced on expired lease must return false");

    let expired_retry = queue.retry_later_fenced(row.id, token, v2, 1, "test_err").await.unwrap();
    assert!(!expired_retry, "retry_later_fenced on expired lease must return false");
}
...
async fn test_mid_poll_reauth_stale_event_rejected() {
    ...
    services.registry.upsert_registration(&session_1, &reg_input).await.unwrap();
    ...
    services.registry.activate_account_session(
        recipient_did,
        &session_2.id.to_string(),
        &session_2.pds_url,
    ).await.unwrap();

    let current_gen: i64 = sqlx::query_scalar("SELECT auth_generation FROM push_accounts WHERE account_did = $1")
        .bind(recipient_did)
        .fetch_one(&pool)
        .await
        .unwrap();
    assert_eq!(current_gen, 3, "Account generation after logout + reauth must be 3");
    ...
    catbird::services::push::lock::acquire_account_and_device_lock(&mut tx, recipient_did, token).await.unwrap();
    let fast_path_outcome = services.verify_pre_send_fence_tx(
        &mut tx,
        recipient_did,
    ...
    assert_eq!(fast_path_outcome, PreSendFenceOutcome::Revoked, "Fast path must reject stale pre-poll event");

    // Durable queue enqueue and worker processing
    catbird::services::chat_poll::poller::enqueue_push(&pool, &chat_event, 0).await.unwrap();
    let processed = services.process_queue_batch(&state).await.unwrap();
...
async fn test_firehose_candidate_generation_and_reauth_fence() {
    ...
    let stored_gen: i64 = sqlx::query_scalar("SELECT auth_generation FROM push_event_queue WHERE dedupe_key = 'dedupe-firehose-1'")
        .fetch_one(&pool)
        .await
        .unwrap();
...
fn test_shared_writer_lock_keys_identical() {
    ...
    for s in &["test_shared_identifier", "did:plc:same_str"] {
        ...
        assert_ne!(acc_key, dev_key, "Account and device keys must have distinct domain separators");
    }
}
