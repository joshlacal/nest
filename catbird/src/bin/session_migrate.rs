...
async fn run_verify(
    ...
    println!("Scanning keys on source ({})...", redact_redis_url(source_url));
    let src_keys = scan_keys(&mut src, prefix, batch_size).await?;
    println!("Scanning keys on target ({})...", redact_redis_url(target_url));
...
async fn run_backfill(
    ...
    session_keys.sort();
    session_keys.dedup();
    println!("Found {} session keys to inspect for backfill", session_keys.len());
    ...
    for chunk in session_keys.chunks(batch_size) {
        ...
        for k in chunk {
            ...
            let val = serde_json::from_str::<serde_json::Value>(&json)
                .map_err(|e| anyhow::anyhow!("Failed to parse decrypted session JSON for '{k}': {e}"))?;
            let sid = val
                .get("session_id")
                .and_then(|s| s.as_str())
                .ok_or_else(|| anyhow::anyhow!("Session record '{k}' missing 'session_id' field in payload"))?;
...
fn parse_encryption_key(raw_key: &str) -> anyhow::Result<[u8; 32]> {
    ...
    if bytes.len() != 32 {
        anyhow::bail!("Encryption key must be exactly 32 bytes (got {} bytes)", bytes.len());
    }
...
mod tests {
    ...
    async fn test_connect_error_sanitizes_sentinels() {
        ...
        assert!(!err.contains("SECRET_PASSWORD"), "Error must not contain password: {err}");
        assert!(!err.contains("SECRET_SENTINEL"), "Error must not contain sentinel path: {err}");
        assert!(!err.contains("SECRET_QUERY"), "Error must not contain query params: {err}");
        assert!(!err.contains("SECRET_FRAG"), "Error must not contain fragments: {err}");
        assert!(err.contains("redis://127.0.0.1:6379"), "Error must contain safe redacted URL: {err}");
    }

    #[test]
    fn test_write_secure_export_file_mode_and_create_new() {
        let temp_dir = std::env::temp_dir().join(format!("test_export_mode_{}", uuid::Uuid::new_v4()));
        ...
        #[cfg(unix)]
        {
            ...
            assert_eq!(mode, 0o600, "Export file must be created with mode 0600 (owner read/write only)");
        }
    ...
    fn test_write_secure_export_file_rejects_symlink() {
        let temp_dir = std::env::temp_dir().join(format!("test_export_symlink_{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&temp_dir).unwrap();
    ...
    async fn test_backfill_missing_encryption_key_fails_closed() {
        ...
        assert!(err_msg.contains("Encryption key is required"), "Unexpected error: {err_msg}");
    }

    #[tokio::test]
    async fn test_backfill_invalid_encryption_key_fails_closed() {
        let res = run_backfill("redis://127.0.0.1:6379", "catbird:session:", Some("not-valid-hex-or-b64"), 10, true).await;
        assert!(res.is_err());
        let err_msg = res.unwrap_err().to_string();
        assert!(err_msg.contains("Encryption key must be 32 bytes"), "Unexpected error: {err_msg}");
    }
    ...
    fn test_parse_encryption_key_hex_and_base64() {
        ...
        let parsed_hex = parse_encryption_key(hex_key).expect("64-char hex key must parse to 32 bytes");
        ...
        let parsed_b64 = parse_encryption_key(&b64_key).expect("44-char base64 key must parse to 32 bytes");
