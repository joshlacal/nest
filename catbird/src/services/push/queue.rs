...
impl PushQueue {
    ...
    pub async fn delete_fenced(&self, id: i64, lease_token: uuid::Uuid, lease_version: i64) -> Result<bool> {
        let result = sqlx::query(
    ...
    pub async fn is_lease_valid(&self, id: i64, lease_token: uuid::Uuid, lease_version: i64) -> Result<bool> {
        let row = sqlx::query(
            r#"
...
mod tests {
    ...
    impl SqlPushQueueEngine {
            ...
            let mut claimable: Vec<i64> = self
                ...
                .filter(|r| {
                    r.available_at <= now
                        ...
                        && !self.auth_revoked_accounts.get(&r.recipient_did).copied().unwrap_or(false)
                })
                .map(|r| r.id)
                .collect();
            claimable.sort();
        ...
        fn sql_is_lease_valid(&self, id: i64, lease_token: Uuid) -> bool {
            ...
            self.rows
                .get(&id)
                .map(|r| r.lease_token == Some(lease_token) && r.leased_until.map(|u| u >= now).unwrap_or(false))
                .unwrap_or(false)
        }
        ...
        fn sql_delete_fenced(&mut self, id: i64, lease_token: Uuid) -> bool {
            ...
            if let Some(row) = self.rows.get(&id) {
                if row.lease_token == Some(lease_token) && row.leased_until.map(|u| u >= now).unwrap_or(false) {
                    self.rows.remove(&id);
                    return true;
                }
            }
    ...
    fn test_mid_fanout_revocation_cancels_subsequent_sends() {
        ...
        for (idx, dev) in devices.iter().enumerate() {
            ...
            if queue.auth_revoked_accounts.get(recipient_did).copied().unwrap_or(false) {
                break;
            }
            ...
            if idx == 0 {
                queue.auth_revoked_accounts.insert(recipient_did.to_string(), true);
            }
        }
