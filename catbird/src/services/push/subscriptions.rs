...
impl PushSubscriptions {
    ...
    pub async fn list_paginated_profiles_with_endpoint(
        ...
        for chunk in actor_ids.chunks(25) {
            ...
            let mut url = Url::parse(endpoint_url).map_err(|e| ActivityHydrationError::Other(e.into()))?;
            {
            ...
            let stream_remaining_time = match MAX_ACTIVITY_HYDRATION_DURATION.checked_sub(start_time.elapsed()) {
                Some(t) if !t.is_zero() => t,
                _ => {
                    return Err(ActivityHydrationError::DurationExhausted(
                        MAX_ACTIVITY_HYDRATION_DURATION,
                        ...
                        Err(e) => {
                            if e.kind() == std::io::ErrorKind::PermissionDenied {
                                ...
                                let retry_after = msg
                                    ...
                                    .unwrap_or(5);
                                return Err(ActivityHydrationError::RateLimitExceeded { retry_after });
            ...
            let items = payload
                ...
                .ok_or_else(|| ActivityHydrationError::Other(anyhow!("AppView getProfiles response missing 'profiles' array")))?;
            ...
            for item in items {
                let did = item
                    ...
                    .ok_or_else(|| ActivityHydrationError::Other(anyhow!("AppView profile missing 'did' field")))?;

                if !chunk.iter().any(|d| d == did) {
                    return Err(ActivityHydrationError::Other(anyhow!("AppView getProfiles returned unexpected extra DID: {}", did)));
                }
                if chunk_map.insert(did.to_string(), item.clone()).is_some() {
                    return Err(ActivityHydrationError::Other(anyhow!("AppView getProfiles returned duplicate DID: {}", did)));
                }
            }

            for did in chunk {
                let item = chunk_map.get(did).ok_or_else(|| {
                    ActivityHydrationError::Other(anyhow!("Incomplete profile hydration: missing profile for requested DID {}", did))
                })?;
...
mod tests {
    ...
    async fn test_sql_subscription_quota_and_legacy_delete() {
        ...
        let pref_delete = ActivitySubscriptionPreference { post: false, reply: false };
        ...
        let pref_create = ActivitySubscriptionPreference { post: true, reply: true };
        let res = subs.put(&subscriber_did, legacy_invalid_subject, &pref_create).await;
        assert!(res.is_err());
    ...
    async fn test_sql_concurrent_subscription_advisory_lock_race() {
        ...
        query_builder.push_values(entries, |mut b, (sub, subj)| {
            b.push_bind(sub).push_bind(subj).push_bind(true).push_bind(true).push("NOW()").push("NOW()");
        });
        query_builder.build().execute(&pool).await.unwrap();

        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM activity_subscriptions WHERE subscriber_did = $1")
            .bind(&subscriber_did)
            .fetch_one(&pool)
            .await
            .unwrap();
        ...
        let pref = ActivitySubscriptionPreference { post: true, reply: true };
        let p1 = pref.clone();
        let p2 = pref.clone();

        let t1 = tokio::spawn(async move {
            s1.put(&sub1, "did:plc:final_slot_1", &p1).await
        });
        let t2 = tokio::spawn(async move {
            s2.put(&sub2, "did:plc:final_slot_2", &p2).await
        });
        ...
        assert_eq!(successes, 1, "Exactly one writer should succeed into the final subscription slot");
        assert_eq!(failures, 1, "The second writer must fail on quota exceeded");

        let final_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM activity_subscriptions WHERE subscriber_did = $1")
            .bind(&subscriber_did)
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    async fn test_sql_keyset_pagination_stability() {
        ...
        subs.put(
            &subscriber_did,
            "did:plc:item01",
            &ActivitySubscriptionPreference { post: false, reply: false },
        )
        .await
        .unwrap();
        subs.put(
            ...
            &ActivitySubscriptionPreference { post: true, reply: true },
        )
        .await
        .unwrap();
    ...
    async fn test_hydration_rejects_incomplete_profiles_did_coverage() {
        ...
        let res = subs
            ...
            .await;
        let err_msg = res.unwrap_err().to_string();
        assert!(err_msg.contains("Incomplete profile hydration") || err_msg.contains("missing profile for requested DID"));
    ...
    async fn test_hydration_succeeds_with_complete_coverage_and_publishes_keyset_cursor() {
        ...
        subs.put(
            &subscriber_did,
            "did:plc:target_a",
            &ActivitySubscriptionPreference { post: true, reply: false },
        )
        .await
        .unwrap();
        subs.put(
            &subscriber_did,
            "did:plc:target_b",
            &ActivitySubscriptionPreference { post: false, reply: true },
        )
        .await
        .unwrap();
        subs.put(
        ...
        assert_eq!(profiles[0]["viewer"]["activitySubscription"]["reply"], false);
    ...
    fn test_hydration_constants_and_limits() {
        ...
        assert_eq!(MAX_ACTIVITY_HYDRATION_DURATION, std::time::Duration::from_secs(30));
    }
    ...
    async fn test_hydration_enforces_active_stream_semaphore_fail_closed() {
        ...
        assert!(err.contains("Active stream concurrency limit reached"), "Error was: {err}");
    ...
    async fn test_hydration_enforces_session_byte_rate_limit() {
        ...
        assert!(res.is_err(), "Must fail when session byte rate limit is exceeded");
        let err = res.unwrap_err().to_string();
        assert!(err.contains("Rate limit exceeded") || err.contains("PermissionDenied"), "Error was: {err}");
    ...
    async fn test_hydration_error_rate_limit_maps_to_429_with_retry_after() {
        ...
        let err: crate::error::AppError = ActivityHydrationError::RateLimitExceeded { retry_after: 15 }.into();
        let resp = err.into_response();
    ...
    async fn test_hydration_error_byte_budget_maps_to_429() {
        ...
        let err: crate::error::AppError = ActivityHydrationError::ByteBudgetExceeded(2 * 1024 * 1024).into();
        let resp = err.into_response();
        assert_eq!(resp.status(), StatusCode::TOO_MANY_REQUESTS);
