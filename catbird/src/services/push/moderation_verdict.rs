    ...
    pub fn try_from_viewer_json(viewer: Option<&serde_json::Value>) -> Result<Self> {
        ...
        if let Some(val) = viewer.get("muted") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.muted must be a boolean, got {:?}", val)),
            }
        }
        ...
        if let Some(val) = viewer.get("mutedByList") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.mutedByList must be an object, got {:?}", val)),
            }
        }
        ...
        if let Some(val) = viewer.get("blocking") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.blocking must be a non-empty string, got {:?}", val)),
            }
        }
        ...
        if let Some(val) = viewer.get("blockedBy") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.blockedBy must be a boolean, got {:?}", val)),
            }
        }
        ...
        if let Some(val) = viewer.get("blockingByList") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.blockingByList must be an object, got {:?}", val)),
            }
        }
        ...
        if let Some(val) = viewer.get("mutedOnlyReposts") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.mutedOnlyReposts must be a boolean, got {:?}", val)),
            }
        }
        ...
        if let Some(val) = viewer.get("mutedOnlyQuoteposts") {
            match val {
                ...
                _ => return Err(anyhow!("viewerState.mutedOnlyQuoteposts must be a boolean, got {:?}", val)),
            }
        }
        Ok(Self {
            muted,
...
impl ActorModerationResolver {
    ...
    async fn fetch_and_persist_live(
        ...
        let account = sqlx::query(
            r#"
            SELECT session_id, pds_url, auth_revoked_at, moderation_generation
            FROM push_accounts
            WHERE account_did = $1
            "#,
        )
        ...
        .await
        {
            ...
            Err(err) if super::is_auth_revocation_error(&err) => {
                ...
                let registry = super::registry::PushRegistry::new(self.db_pool.clone(), String::new());
                let _ = registry
