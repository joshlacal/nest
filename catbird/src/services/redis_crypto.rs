...
pub fn open_v2_with_metadata(
    ...
    if let Some((header_b64, payload_b64)) = rest.split_once(':') {
        ...
        if metadata.record_class != expected_record_class
            || metadata.redis_key != expected_redis_key
        {
...
mod tests {
    ...
    fn test_aad_binding_and_transplantation_rejection() {
        ...
        let aad_session = build_aad(
            "session",
            "catbird:v2:session:fp1",
            Some("did:plc:alice"),
            None,
        );
        let aad_upgrade = build_aad(
            "upgrade_candidate",
        ...
        let aad_wrong_key = build_aad(
            "session",
            "catbird:v2:session:fp2",
            Some("did:plc:alice"),
            None,
        );
        assert_eq!(
    ...
    fn test_legacy_v1_dual_read_support() {
        ...
        let (decrypted_str, meta, is_v1_meta) =
            open_v2_with_metadata(&key, &v1_sealed, "session", "catbird:v2:session:fp1").unwrap();
        assert!(is_v1_meta);
    ...
    fn test_v2_envelope_metadata_aad_roundtrip() {
        ...
        let err_class =
            open_v2_with_metadata(&key, &sealed, "session", "catbird:v2:upgrade_candidate:fp1")
                .unwrap_err();
    ...
    fn test_envelope_metadata_header_tampering_rejected() {
        ...
        let header_json: Vec<u8> = base64::engine::general_purpose::STANDARD
            .decode(header_b64)
            .unwrap();
        ...
        let tampered_header_b64 = base64::engine::general_purpose::STANDARD
            .encode(serde_json::to_vec(&tampered_meta).unwrap());
