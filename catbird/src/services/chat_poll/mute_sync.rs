...
pub(crate) async fn sync_mutes_for_account(
    ...
    let (session_id, pds_url) =
        match tokio::time::timeout(MAX_MUTE_SYNC_DURATION, lookup_push_account(db_pool, did)).await
        {
            Ok(res) => res?,
            Err(_) => {
    ...
    .await
    {
        ...
        Err(_) => {
            ...
            return Err(anyhow!(
                "Mute sync session resolution timed out for {}",
                did
            ));
        }
    };
    ...
    if completed_cleanly {
        let commit_remaining = match MAX_MUTE_SYNC_DURATION.checked_sub(start_time.elapsed()) {
            ...
            _ => {
                ...
                return Err(anyhow!(
                    "Mute sync duration budget exhausted before commit for {}",
                    did
                ));
            }
        };
...
mod tests {
    ...
    async fn test_cursor_cycle_detection() {
        ...
        let start_time = std::time::Instant::now();
        let res = sync_mutes_for_session(
            &state, &scheduler, &session, &dpop_data, &test_did, start_time,
        )
        .await;
        ...
        let count: (i64,) =
            sqlx::query_as("SELECT COUNT(*) FROM chat_muted_convos WHERE account_did = $1")
                .bind(&test_did)
                .fetch_one(&pool)
                .await
                .unwrap();
    ...
    async fn test_cursor_length_validation() {
        ...
        assert_eq!(
            count.0, 0,
            "No mutes must be committed on oversized cursor abort"
        );
    }
    ...
    async fn test_mute_sync_all_items_counted_against_budget() {
        ...
        assert!(
            res.is_err(),
            "Must fail when total returned items exceed entry budget"
        );
        let err = res.unwrap_err().to_string();
        assert!(err.contains("did not complete cleanly"), "Error was: {err}");
        ...
        assert_eq!(
            count.0, 0,
            "No mutes must be committed when entry budget is exceeded"
        );
    }
    ...
    async fn test_mute_sync_invalid_item_aborts_without_partial_staging() {
        ...
        assert!(
            res.is_err(),
            "Must fail when invalid/malformed conversation ID is encountered"
        );
        let err = res.unwrap_err().to_string();
        assert!(err.contains("did not complete cleanly"), "Error was: {err}");
        ...
        assert_eq!(
            count.0, 0,
            "No mutes must be committed when invalid item is encountered"
        );
    }
    ...
    async fn test_mute_sync_enforces_duration_deadline_and_aborts_without_committing() {
        ...
        let start_time =
            std::time::Instant::now() - (MAX_MUTE_SYNC_DURATION - Duration::from_millis(50));
        ...
        assert!(
            res.is_err(),
            "Must fail when upstream delay exceeds remaining duration"
        );
        let err = res.unwrap_err().to_string();
        assert!(
            err.contains("did not complete cleanly") || err.contains("timed out"),
            "Error was: {err}"
        );
