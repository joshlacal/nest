...
async fn send_get_log_request(
    ...
    let response = state.raw_http_client.get(url).headers(headers).send().await?;
    ...
    let decoded_body =
        decompress_bounded(&encoded_body, content_encoding.as_deref(), max_bytes)?;
...
mod tests {
    ...
    async fn test_active_stream_semaphore_fail_closed_on_exhaustion() {
        ...
        assert!(res.is_err(), "getLog must fail when active stream semaphore is exhausted");
    ...
    async fn test_send_get_log_routes_through_task1_byte_accounting() {
        ...
        assert!(res.is_err(), "getLog must fail when rate limit budget is exceeded");
        let err_msg = res.unwrap_err().to_string();
        assert!(
    ...
    async fn test_send_get_log_enforces_content_length_ceiling() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(
                ResponseTemplate::new(200)
                    .set_body_bytes(vec![0u8; 6_000_000]),
            )
            .mount(&pds)
            .await;
        ...
        assert!(res.is_err(), "getLog must reject Content-Length > MAX_GET_LOG_BODY_SIZE");
        let err_msg = res.unwrap_err().to_string();
    ...
    async fn test_fetch_log_page_real_nonce_retry_and_cumulative_budget() {
        ...
        // Scenario 1: Normal successful retry under cumulative budget
        {
            ...
            match outcome {
                FetchLogOutcome::Success { status, next_nonce, .. } => {
        ...
        // Scenario 2: Attempt 1 challenge body consumes entire 5MB budget -> cumulative bail before retry
        {
            ...
            let full_budget_body = format!("{{\"error\":\"use_dpop_nonce\",\"p\":\"{}\"}}", "e".repeat(padding_len)).into_bytes();
        ...
        // Proves that retry receives the reduced remaining_budget and fails closed.
        {
            ...
            let attempt1_body = format!("{{\"error\":\"use_dpop_nonce\",\"p\":\"{}\"}}", "a".repeat(padding_len)).into_bytes();
    ...
    async fn test_prime_pass_enforces_timeout_deadline_and_reschedules_current_item() {
        ...
        scheduler.enroll_account(&test_did, &pds_host).await.unwrap();
        ...
        let prime_start = std::time::Instant::now() - (MAX_PRIME_PASS_DURATION - Duration::from_millis(50));
        ...
        assert!(res.is_ok(), "Prime pass timeout must yield cleanly with Ok(()) without panicking or erroring");

        // Verify account primed_at remains NULL in DB
        let primed_at: (Option<time::OffsetDateTime>,) = sqlx::query_as(
            "SELECT primed_at FROM chat_poll_state WHERE account_did = $1"
        )
        .bind(&test_did)
        .fetch_one(&pool)
        .await
        .unwrap();
        assert!(primed_at.0.is_none(), "Account must not be marked primed when timed out");
    ...
    async fn test_send_get_log_rejects_unsupported_content_encoding() {
        ...
        assert!(res.is_err(), "Must reject unsupported content-encoding zstd");
        let err_msg = res.unwrap_err().to_string();
        assert!(err_msg.contains("Unsupported Content-Encoding: zstd"));
    }
    ...
    async fn test_prime_pass_bounds_logs_item_count_and_batches_watermarks() {
        ...
        let count: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM chat_notified_watermarks WHERE account_did = $1"
        )
        .bind(&test_did)
        .fetch_one(&pool)
        .await
        .unwrap();
        assert_eq!(count.0, 1000, "Must bound watermark items to MAX_PRIME_LOGS_PER_PAGE (1000)");
