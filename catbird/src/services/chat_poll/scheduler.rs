...
impl ChatPollScheduler {
    ...
    pub async fn bump_watermarks_batch(
        ...
        let (convo_ids, revs): (Vec<String>, Vec<String>) = watermarks
            .iter()
...
mod tests {
    ...
    async fn test_sql_chat_mute_quota_and_legacy_unmute() {
        ...
        assert!(scheduler
            .is_convo_muted(&did, legacy_invalid_id)
            .await
            .unwrap());
        ...
        let res = scheduler
            .set_convo_muted(&did, legacy_invalid_id, true)
            .await;
        assert!(res.is_err());
        ...
        let count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM chat_muted_convos WHERE account_did = $1")
                .bind(&did)
                .fetch_one(&pool)
                .await
                .unwrap();
    ...
    async fn test_sql_concurrent_chat_mute_advisory_lock_race() {
        ...
        let t1 =
            tokio::spawn(async move { s1.set_convo_muted(&did1, "final_convo_1", true).await });
        let t2 =
            tokio::spawn(async move { s2.set_convo_muted(&did2, "final_convo_2", true).await });
        ...
        assert_eq!(
            successes, 1,
            "Exactly one writer should succeed into the final slot"
        );
