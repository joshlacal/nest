...
use super::redis_crypto::{
    open_strict, open_v2_with_metadata, seal_v2_with_metadata, EnvelopeMetadata,
};
...
impl RedisAuthStore {
    ...
    pub fn update_reconcile_cursor(
        &self,
        start_cursor: u64,
        end_cursor: u64,
        failed: usize,
            ...
            None => {
                ...
                loop {
                    ...
                    for key_name in did_keys {
                        let Some(rest) =
                            key_name.strip_prefix(&format!("{}session:{}_", self.key_prefix, did))
                        else {
                            continue;
                        };
                        let candidate_hmac = rest.to_string();
                        let candidate_retired_k =
                            format!("{}upgrade_retired:{}", self.key_prefix, candidate_hmac);
                        ...
                        let Ok(cand_session) = serde_json::from_str::<ClientSessionData>(&json)
                        else {
                            continue;
                        };
                    ...
                    if !alt_retired {
                            ...
                            .await
                        {
                            if let Ok(alt_json) = open_strict(&self.encryption_key, &alt_enc) {
                                if let Ok(alt_session) =
                                    serde_json::from_str::<ClientSessionData>(&alt_json)
                                {
                                    let alt_computed =
                                        crate::services::push::registry::session_fingerprint(
                                            alt_session.session_id.as_str(),
                                        );
                                    if alt_computed == expected_fp
                                        || alt_session.session_id.as_str() == expected_fingerprint
                                    {
                                        let install_script =
    ...
    pub async fn reconcile_legacy_sessions(
        &self,
    ) -> Result<ReconciliationOutcome, SessionStoreError> {
        let mut conn = self.redis.clone();
...
impl ClientAuthStore for RedisAuthStore {
    ...
    async fn get_session<D: jacquard_common::BosStr + Send + Sync>(
        ...
        if let Some(encrypted) = data {
            ...
            if is_v1 {
                let meta =
                    EnvelopeMetadata::new("session", &hashed_session_key, Some(did.as_str()), None);
                let v2_encrypted = seal_v2_with_metadata(&self.encryption_key, &json, &meta)
                    .map_err(|e| {
                        other_err(&format!("Session reseal failed during migration: {e}"))
                    })?;
    ...
    async fn get_auth_req_info(
        ...
        match data {
            Some(encrypted) => {
                let (json, _metadata, _) =
                    open_v2_with_metadata(&self.encryption_key, &encrypted, "auth_req_info", &key)
...
mod tests {
    ...
    fn make_test_session_data(did_str: &str, session_id_str: &str) -> ClientSessionData {
        ...
        ClientSessionData {
            ...
            host_url: jacquard_common::deps::fluent_uri::Uri::parse("https://pds.example.com")
                .unwrap()
                .to_owned(),
            authserver_url: "https://pds.example.com".to_string().into(),
            authserver_token_endpoint: "".into(),
            authserver_revocation_endpoint: None,
            scopes: jacquard_oauth::scopes::Scopes::new(smol_str::SmolStr::new_static(
    ...
    fn test_get_session_corrupt_payload_simulation() {
        ...
        let parse_partial: Result<ClientSessionData, _> = serde_json::from_str(&decrypted_partial);
    ...
    fn test_clean_cutover_invalid_or_old_bearer_401_middleware_model() {
        ...
        impl MiddlewareSim {
            fn authenticate(&self, session_id_header: &str) -> Result<String, MockAuthError> {
                ...
                let did = Did::new(did_str.as_str())
                    .map_err(|e| MockAuthError::Internal(e.to_string()))?;
    ...
    fn test_logout_retire_atomically_revokes_winner_candidate_lineage_logic() {
        ...
        assert_eq!(
            store.get(&old_retired_k).map(|s| s.as_str()),
            Some("logout")
        );
        ...
        assert_eq!(
            store.get(&winner_retired_k).map(|s| s.as_str()),
            Some("logout")
        );
                    ...
                    if let Some(rest) = k.strip_prefix(&session_prefix) {
                        if let Some((did, hmac_fp)) = rest.rsplit_once('_') {
                            if let Ok(json) = open_strict(&self.key, v) {
                                if let Ok(session) =
                                    serde_json::from_str::<ClientSessionData>(&json)
                                {
                                    let sha_fp =
                                        crate::services::push::registry::session_fingerprint(
                                            session.session_id.as_str(),
                                    ...
                                    let fp_idx =
                                        format!("{}session_fp_index:{}", self.prefix, sha_fp);
            ...
            fn resolve_session(
                ...
                if computed_fp != expected_fp && session.session_id.as_str() != expected_fingerprint
                {
                    ...
                    return Err(other_err(
                        "inner fingerprint mismatch (stale index evicted, retryable)",
                    ));
                }
    ...
    fn test_resolve_session_self_heals_missing_fp_index() {
        ...
        assert!(
            recovered.is_some(),
            "Self-healing must recover session from un-indexed DID keys"
        );
    ...
    fn test_reconcile_legacy_sessions_observability_and_counts() {
        ...
        store.insert(
            format!("{prefix}upgrade_retired:{hmac2}"),
            "retired".to_string(),
        );
        ...
        store.insert(
            format!("{prefix}session:{did3}_{hmac3}"),
            "not-encrypted-blob".to_string(),
        );
        ...
        assert_eq!(
            reconciled, 1,
            "Exactly 1 valid session should be reconciled"
        );
        assert_eq!(skipped, 1, "Exactly 1 retired session should be skipped");
        assert_eq!(
            failed, 1,
            "Exactly 1 corrupt session should be counted as failed"
        );
    }
    ...
    fn test_resolve_session_self_healing_iterates_multi_page_scan_cursor() {
        ...
        let scan_pages: std::collections::HashMap<u64, (u64, Vec<String>)> =
            std::collections::HashMap::from([
                (
                    0,
                    (
                        42,
                        vec![format!("{prefix}session:did:plc:other_user1_hmac1")],
                    ),
                ),
        ...
        loop {
            let (next_cursor, did_keys) = scan_pages
                .get(&scan_cursor)
                .cloned()
                .unwrap_or((0, Vec::new()));
            scan_cursor = next_cursor;
        ...
        assert!(
            recovered.is_some(),
            "Self-healing must iterate multi-page SCAN cursor to recover session"
        );
        let (recovered_session, recovered_hmac) = recovered.unwrap();
        assert_eq!(recovered_session.session_id, session_id);
    ...
    fn test_reconcile_legacy_sessions_outcome_reporting_and_rearm_tracking() {
        ...
        assert!(
            !outcome_truncated.is_complete,
            "Truncated scan must report is_complete = false to trigger timer re-arm"
        );
        ...
        assert!(
            outcome_failed.failed > 0,
            "Failures must be tracked to trigger timer re-arm"
        );
    }
    ...
    fn test_self_heal_scan_bounds_pages_on_miss_without_infinite_keyspace_walk() {
        ...
        assert!(!should_terminate_self_heal_scan(
            false,
            1,
            MAX_SELF_HEAL_SCAN_PAGES - 1
        ));
        ...
        assert!(should_terminate_self_heal_scan(
            false,
            1,
            MAX_SELF_HEAL_SCAN_PAGES
        ));
        assert!(should_terminate_self_heal_scan(
            false,
            100,
            MAX_SELF_HEAL_SCAN_PAGES + 5
        ));
    ...
    fn test_fingerprint_mismatch_evicts_stale_index_and_is_retryable() {
        ...
        let enc = store
            .get(&format!("{prefix}session:{did}_{old_hmac_fp}"))
            .unwrap();
    ...
    fn test_o1_fp_index_no_scan_constant_time_with_large_keyspace() {
        ...
        let enc = seal_strict(
            &TEST_KEY,
            &serde_json::to_string(&target_sess_data).unwrap(),
        )
        .unwrap();
        ...
        let enc_legacy = seal_strict(
            &TEST_KEY,
            &serde_json::to_string(&legacy_sess_data).unwrap(),
        )
        .unwrap();
        sim.store.borrow_mut().insert(
