...
impl ByteRateLimiter {
    ...
    pub async fn check_and_record(&self, key: &str, bytes: u64, config: &ByteRateLimitConfig) -> Result<(), u64> {
        let now = Instant::now();
        let mut entries = self.entries.write().await;

        let entry = entries.entry(key.to_string()).or_insert(ByteRateLimitEntry {
            bytes: 0,
            window_start: now,
        });
        ...
        if entry.bytes + bytes > config.max_bytes {
            let retry_after = config.window.as_secs().saturating_sub(now.duration_since(entry.window_start).as_secs());
            return Err(retry_after.max(1));
        }
    ...
    pub fn start_cleanup_task(self: Arc<Self>) {
        tokio::spawn(async move {
            ...
            loop {
                ...
                self.session_byte_limiter.cleanup(Duration::from_secs(120)).await;
                self.global_byte_limiter.cleanup(Duration::from_secs(120)).await;
