...
pub fn sanitize_error_message(msg: &str) -> String {
    ...
    if let Ok(url_creds_re) = regex::Regex::new(r#"(https?|redis|postgres|postgresql)://([^:@\s]+):([^@\s]+)@"#) {
        sanitized = url_creds_re.replace_all(&sanitized, "$1://[REDACTED]:[REDACTED]@").to_string();
    }

    // Redact bearer tokens / authorization headers
    if let Ok(bearer_re) = regex::Regex::new(r#"(?i)(bearer\s+|authorization:\s*bearer\s+)([A-Za-z0-9\-_.~+/]+=*)"#) {
        sanitized = bearer_re.replace_all(&sanitized, "$1[REDACTED_TOKEN]").to_string();
    }

    // Redact refresh_token, access_token, client_secret in JSON or key-value format
    if let Ok(token_re) = regex::Regex::new(r#"(?i)("(?:access_token|refresh_token|token|client_secret|password|secret)"\s*:\s*)"[^"]+""#) {
        sanitized = token_re.replace_all(&sanitized, r#"$1"[REDACTED]""#).to_string();
    }
